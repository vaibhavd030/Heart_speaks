...
def get_vector_store() -> Chroma:
    ...
    embeddings = OpenAIEmbeddings(
        api_key=SecretStr(settings.openai_api_key), model="text-embedding-3-large"
    )
    return Chroma(
...
def ingest_data(data_path: str = settings.data_dir) -> Chroma:
    ...
    for doc in docs:
        ...
        filename = (
            os.path.relpath(source, data_path) if source != "Unknown" else "Unknown"
        )
        ...
        doc.metadata["personality"] = (
            docs_by_file.get(filename, {}).get("author", "Unknown")
            if docs_by_file
            else author
        )
        ...
        docs_by_file[filename]["text"] = (
            str(docs_by_file[filename]["text"]) + str(doc.page_content) + "\n\n"
        )
        docs_by_file[filename]["pages"] = int(docs_by_file[filename]["pages"]) + 1
    ...
    for i in range(0, len(splits), batch_size):
        ...
        ids = [
            hashlib.md5(
                f"{doc.metadata.get('source_file', 'unknown')}:{doc.page_content}".encode()
            ).hexdigest()
            for doc in batch
        ]
        vectorstore.add_documents(batch, ids=ids)
        logger.info(
            f"Added batch {int(i/batch_size) + 1} of {(len(splits) + batch_size - 1) // batch_size}"
        )
