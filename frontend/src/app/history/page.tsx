...
import { ArrowLeft, MessageSquare, Search, ChevronDown, ChevronUp, User, Trash2, Clock, Calendar, AlertCircle, RefreshCw } from 'lucide-react';
        ...
        <AuthGuard>
            <div className="min-h-screen bg-parchment-light">
                ...
                <main className="max-w-4xl mx-auto px-4 py-8 space-y-3">
                    {isLoading ? (
                        <div className="text-center py-24 text-sepia-light italic flex flex-col items-center gap-4">
                        ...
                        <div className="text-center py-24 text-sepia-light italic">
                            {searchQuery ? "No entries match your search." : "Your spiritual journey is just beginning. Ask SAGE a question."}
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {filtered.map((log) => (
                                ...
                                >
                                    ...
                                    >
                                        {/* User avatar - matching admin style */}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 flex-wrap mb-1">
                                                ...
                                                <span className="text-xs text-sepia-light flex items-center gap-1 uppercase tracking-wider font-heading font-bold">
                                            ...
                                            <p className="text-sm text-sepia-dark font-medium truncate">
                                                <span className="text-gold-accent/70 font-bold uppercase text-[10px] mr-2">Q:</span>
                                                {log.question}
                                            </p>
                                            <p className="text-sm text-sepia-light italic truncate mt-0.5">
                                                 <span className="text-sepia-light/70 font-bold uppercase text-[10px] mr-2">A:</span>
                                                 {log.response?.slice(0, 120)}...
                                            </p>
                                        </div>

                                        <div className="shrink-0 text-sepia-light mt-4">
                                            {expandedId === log.id
                                    ...
                                    {expandedId === log.id && (
                                        <div className="px-6 pb-6 border-t border-gold-accent/10 pt-4 space-y-4">
                                            ...
                                            <div className="rounded-lg bg-gold-accent/5 border border-gold-accent/20 p-4">
                                                <p className="text-[10px] uppercase tracking-widest font-heading font-bold text-gold-accent mb-2">The Inquiry</p>
                                                <p className="text-sm text-sepia-dark font-medium leading-relaxed">{log.question}</p>
                                            </div>

                                            <div className="rounded-lg bg-parchment-light border border-gold-accent/10 p-4">
                                                <p className="text-[10px] uppercase tracking-widest font-heading font-bold text-sepia-light mb-2">The Guidance Received</p>
                                                <p className="text-sm text-sepia-dark leading-relaxed whitespace-pre-wrap italic">{log.response}</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
