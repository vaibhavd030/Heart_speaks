...
class QAPair(BaseModel):
    question: str = Field(
        description="A question that can be fully answered using ONLY the provided text."
    )
    ground_truth: str = Field(
        description="The correct answer to the question based ONLY on the provided text."
    )


def generate_synthetic_dataset(
    num_questions: int = 50, output_file: str = "eval_dataset.json"
) -> None:
    ...
    logger.info(
        f"Found {len(docs)} chunks in the vector store. Sampling for dataset generation..."
    )
    ...
    llm = ChatOpenAI(
        model="gpt-4o-mini", temperature=0.7, api_key=SecretStr(settings.openai_api_key)
    )
    ...
    if os.path.exists(output_path):
        with open(output_path) as f:
            try:
                dataset = json.load(f)
                logger.info(
                    f"Loaded {len(dataset)} existing questions from {output_file}"
                )
    ...
    for i, doc in enumerate(sampled_docs):
        try:
            ...
            qa: QAPair = qa_result
            dataset.append({"question": qa.question, "ground_truth": qa.ground_truth})
            generated_count += 1
    ...
    logger.info(
        f"Successfully added {generated_count} questions. Total dataset size: {len(dataset)}. Saved to {output_path}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num", type=int, default=5, help="Number of questions to generate"
    )
